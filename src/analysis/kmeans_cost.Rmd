...
```
# Testing kmeans on different bgs
```{r}
k_tb <-readRDS(file = file.path(project_dir, "data/output/kmeans_clusters/20221205_1846__11clusters_100nstart_scaled.rds"))
...
```{r}
  # Preparing list 
  query <- list()
  for (c in unique(k_tb$kmeans)) {
    query[[c]] <- rownames(dplyr::filter(k_tb, kmeans == c))  
  }
